...
amp = dlmread('amplitudev5/amp267.dat');
%randshift = dlmread('../focussx50y50v3r/randshift.dat');
randshift = zeros(length(modemat(3,:)),137);
%amps = ones(137,182);
%ampp = ones(137,182);
cosp = zeros(length(modemat(3,:)),375);
sinp = zeros(length(modemat(3,:)),375);
...
wvfun = zeros(length(modemat(3,:)),110,110);
%wvfunp = zeros(length(modemat(3,:)),110,110);
for i = 1:length(modemat(3,:))
	wvfun(i,:,:) = Modefun(modemat(1,i),modemat(2,i),Lambda);
...
for i = 1:length(modemat(3,:))
	a1 = imrotate(squeeze(wvfun(i,:,:)),angle(1,i));
